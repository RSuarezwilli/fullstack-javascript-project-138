...
return tasks.run().then(() => {
  return axios.get(pageUrl).then((response) => {
    ...
    // Aquí iría tu lógica de reemplazar paths y guardar archivo
    return fs.mkdir(fullOutputDirname, { recursive: true })
      .then(() => fs.writeFile(path.join(fullOutputDirname, outputFilename), $.html()))
      .then(() => {
        log('Página y recursos guardados en', fullOutputDirname);
